...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = Emps.Where(emp => emp.Salary == Emps.Max(e => e.Salary)).ToList();
            return result;
        }
        ...
        public static IEnumerable<object> Task6()
        {
            IEnumerable<object> result = Emps
                ...
                .Select(joined => new {joined.emp.Ename,joined.emp.Job, joined.dept.Dname})
                .ToList();
            return result;
        }
        ...
        public static IEnumerable<object> Task11()
        {
            IEnumerable<object> result = Depts
                .GroupJoin(Emps, dept => dept.Deptno, emp => emp.Deptno, (dept, emps) => new {dept,emps})
                .Where(joined => joined.emps.Count() > 1)
                .Select(joined => new {name = joined.dept.Dname, numOfEmployees = joined.emps.Count()})
                .ToList();
            return result;
        }
        ...
        {
            int result = arr.GroupBy(num => num).Single(group => group.Count()%2==1).Single();
            //result=
            return result;
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            IEnumerable<Dept> result = Depts
                .GroupJoin(Emps, dept => dept.Deptno, emp => emp.Deptno, (dept, emps) => new { dept, emps })
                .Where(joined => joined.emps.Count() == 5 || joined.emps.Count() == 0)
                .Select(joined => joined.dept)
        ...
        {
            IEnumerable<Dept> result = Depts
                .Join(Emps, depts => depts.Deptno, emps => emps.Deptno, (emps, depts) => (emps, depts))
                .ToList();
            //result =
            return result;
        }

        //Pytanie czy te ostatnie dwa taski to błąd czy to tak ma być? jeśli ma tak być to jak rozwiązać ten problem?
    }
